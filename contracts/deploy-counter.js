...
async function deployContract(options) {
  ...
  try {
    ...
    const abiPath = './managed/abi/counter.json';
    try {
      const abi = JSON.parse(readFileSync(abiPath, 'utf8'));
      console.log('✅ Contract ABI found:', abi.contractName);
    } catch (e) {
      console.error('❌ Contract not compiled. Run: node contracts/compile-counter.js contracts/counter.compact');
      return { success: false, error: 'Contract not compiled' };
    }
    
    // Get private key from environment (or generate mock for demo)
    const privateKey = process.env.MIDNIGHT_PRIVATE_KEY || randomBytes(32).toString('hex');
    if (!process.env.MIDNIGHT_PRIVATE_KEY) {
      console.log('⚠️  No MIDNIGHT_PRIVATE_KEY set, using generated key for demo');
    }
    ...
    writeFileSync('./managed/counter-deployment-info.json', JSON.stringify(deploymentInfo, null, 2));
    console.log('📁 Deployment info saved to managed/counter-deployment-info.json');
